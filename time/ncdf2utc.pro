...
; Name        : NCDF2UTC()
;               
; Purpose     : Convert netCDF time values to CDS UTC time format
;               
; Explanation : This procedure converts netCDF milliseconds since 1970-01-01
;               into TAI seconds, and then formats this into one of the CDS UTC
;               time formats via TAI2UTC.
;
; Use         : UTC = NCDF2UTC(TIME)
;    
...
;
;				ERRMSG = ''
;				RESULT = NCDF2UTC( TIME, ERRMSG=ERRMSG )
;				IF ERRMSG NE '' THEN ...
;
...
;               
; Prev. Hist. : Based partially on cdf2utc.pro
;
...
;-            
;
function ncdf2utc, time, errmsg=errmsg, _extra=_extra
on_error, 2
;
;  Check the input parameter.
;
if n_params() eq 0 then begin
    message = 'Syntax: UTC = NCDF2UTC(TIME)'
    goto, handle_error
endif
...
return, tai2utc(tai, /nocorrect, _extra=_extra)
;
;  Error handling point.
;
handle_error:
    if n_elements(errmsg) eq 0 then message, message else $
      errmsg = 'NCDF2UTC: ' +message
    return, -1
;
end
