...
; July 2020, today determined from a call to systime now.
;------------------------------------------------------------------------
;
...
; Rschwartz70@gmail.com July 2020, updated to run with Vers 9.0.1
;  removed Thomas polynomial computation and plots, other updates
;  this code should not be used in the future. superceded by goes_chianti_response.pro
;  The same methodology is used but the management of the products has been updated
;  and simplified.
...
pro today, yyyymmdd
  ...
  yyyymmdd = anytim(fid='sys',systime(/sec),/ccsds,/date)
...
;updated to use rd_ascii, July 2020, RAS
...
;updated abundance file name for Vers 9.0.1
...
; MODIFICATION HISTORY:
;     SW 2005 Jan
;     2-july-2020, RAS, remove plotting at Thomas formulations for polynomial fits
;-
;
pro fold_spec_resp, nsat, plotspec=plotspec, last_only = last_only, $
  scale = scale, fshort_cor = fshort_cor,  flong_cor = flong_cor

  scale =  1.d-27*1.d-3/((1.496d13)^2)*1.d55

  ; get chianti version
  chianti_version, vers
  default, last_only, 0
  nsat_start = last_only ? nsat : 1
  ...
  for sat=nsat_start,nsat do begin ; GOES satellite number
    ...
    fshort_pho = dblarr(npho) & flong_pho=fshort_pho
    dlambda = 0.01 ;2000 bins from 0 to 20 Angstrom
    for i = 0, npho-1 do begin
      restgen, file=flpho[i],struct=spc
      ; integrate continuum
      fshort_pho[i]= dlambda * total(spc.lambda,ts*spc.spectrum)/gs
      flong_pho[i] = dlambda * total(spc.lambda,tl*spc.spectrum)/gl
    end


    fshort_cor=dblarr(ncor) & flong_cor=fshort_cor
    for i= 0, ncor-1 do begin
      restgen, file=flcor[i],struct=spc

      ; integrate continuum, int_tabulated(previous) unnecessary. 2000 wavelength
      ; bins is as fine as needed without weighting function in int_tabulated
      fshort_cor[i] = dlambda * total(spc.lambda,ts*spc.spectrum)/gs
      flong_cor[i]  = dlambda * total(spc.lambda,tl*spc.spectrum)/gl
    end
