...
pro rd_goes,times,data,err=err,trange=trange,count=count,tai=tai,$
            _ref_extra=extra,status=status,verbose=verbose,gdata=gdata,widget=widget,$
...
if remote then begin
 server=goes_server(response_code=rcode,err=err,verbose=verbose)
 if is_string(err) then begin
  sock_error,server,response_code=rcode,err=err,verbose=verbose
