pro rd_week_file, input10, input20, data_type, data, orb_pointer, vnum=vnum, $
  ...
  ibyt=ibyt, filename=infil, qstop=qstop, err=err,$
...
;   19-Jan-2012, Zarro (ADNET) - fixed potential path problem in
;                                sock_find   
;   20-Dec-2016, Zarro (ADNET) - ensured /USE_NETWORK is default
;-
;
...
if ((typ lt 7) && (typ ne 0)) then $
  if ((max(input1) gt 54) || (n_elements(input1) eq 7)) then begin
...
siz = size(input1)
typ = siz[siz[0] + 1]
if ((typ eq 7) || (typ eq 8)) then begin  ;entered a start/end time
  ...
  if (infil eq '') then begin
      ...
      if verbose then message, 'No input file for week ' + wid + ' found in: ' + indir, /info
            ...
            if (count eq 0) then begin
             ...
             server=goes_server(network=network,_extra=extra,verbose=verbose)
			 if network then begin
              spos1=strpos(infil,'/')
              spos2=strpos(infil,'\')
              if (spos1 eq 0) || (spos2 eq 0) then infil=strmid(infil,1,strlen(infil))
              sfile=infil
              if verbose then message,'Downloading '+sfile,/info
              sock_get,sfile,out=temp_dir,verbose=verbose,_extra=extra,/quiet
             endif
    ...
    case strupcase(data_type) of
        ...
        status = 99
        mprint, 'RD_WEEK_FILE: Cannot recognize data_type: '+trim(data_type)
        return
        endelse
    endcase
      end
    ...
    if (ss_temp[0] eq -1) then begin
      ...
      if ((pointer.opt_section gt 0) && (qtimes)) then begin ;orbital pointer information available
...
;
if (keyword_set(qstop)) then stop
if ((itry ne 0) && (status eq 0)) then status = -1
end
...
function weekid, fileid, all=all,count=count,			$
		 ...
		 gt_distpre=gt_distpre, prefix=prefix, vnum=vnum,$
                 verbose=verbose,remote=remote,_extra=extra,err=err
;+
;   Name: weekid
;
...
if is_dir(comdir) then gen=file_search(gfiles,count=count)

if (count eq 0) || remote then begin
 ...
 if network then begin
  ...
  if verbose && ~remote then mprint,'Local GOES/Yohkoh data not found. Trying remote archive.'
  gen=sock_find(server,name+ext,path=path,count=count,/cache,verbose=verbose,err=err)
 endif else gen=loc_file(name+ext,path=goes_temp_dir(),count=count)
endif
 
;count=n_elements(gen)
if count eq 0 then return,''
if (count eq 1) && gen[0] eq '' then return,''
