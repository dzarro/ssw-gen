...
if count eq 0 then begin
 if verbose then mrpint,err_msg
 if widget then xbanner,err_msg,/append
 error=1
 return
endif
...
server=goes_server(network=network,err=err_msg,/sdac,path=path,response_code=rcode,verbose=verbose)
if is_string(err_msg) then begin
 sock_error,server,response_code=rcode,err=err_msg,verbose=verbose
