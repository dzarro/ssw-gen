...
;       28-Dec-2017, Zarro (ADNET) - added /DISMISS_ONLY
;
; CONTACT:
...
pro xtext, array, font=font, title=title, group=group, modal=modal, $
           ...
           center=center,dismiss_only=dismiss_only

   status=0

   if not allow_windows() then return

   if keyword_set(dismiss_only) then begin 
    no_find=1b & no_save=1b & no_print=1b
   endif
