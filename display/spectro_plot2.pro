...
;
;HISTORY:
;       Kim 09-Sep-2016 - handle range of data to plot better when zlog is set 
...
; of th eoriginal image array. This is really important for large images.
    ...
    ; now deal with logs/exp. this needs the floats values.
        ...
        IF zlog THEN begin
          ; Kim, 9/9/2016, Replaced the following three lines with what's below down to the 'if zlog' endif. 
          ; Adding one didn't seem like a good idea.
;          imagep = Alog( imagep  + abs( minimage ) + 1 )
;          minimage_orig = alog( (minimage_orig + abs(minimage)+1) > 1 )
;          maximage_orig = alog( (maximage_orig + abs( minimage) + 1) > 1 )
;          
          ; do this again, so we use the imagep min value, so we're sure not to end up with negatives
          if ~valid_range(drange) then minimage_orig = minimage
          if minimage_orig le 0. then begin
             qpos = where(imagep gt 0, cpos)
             if cpos lt 4 then begin
              message,'Not enough positive data to show with log_scale. Returning.', /cont
              return
             endif
             minimage_orig = min(imagep[qpos])             
          endif
          minmaximage = [minimage_orig, maximage_orig]
          imagep = alog(imagep > minimage_orig)
          minimage_orig = alog(minimage_orig)
          maximage_orig = alog(maximage_orig)
