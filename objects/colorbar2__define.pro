...
;
; PURPOSE:
;       The purpose of this routine is to implement a COLORBAR2 object
;       class. The ColorBar is rendered in the direct graphics system.
;       This was renamed from colorbar__define 20-Sep-2011 to avoid conflict with IDL V8. routine.
;
; AUTHOR:
...
;       02-Aug-2010, Kim Tolbert.  Change () to {}(for self.position) for IDL Version 8
;       20-Sep-2011, Kim Tolbert.  Renamed to colorbar2 to avoid conflict in IDL V8
;       08-Sep-2016, Kim Tolbert.  If log, don't plot minor ticks
;-

PRO Colorbar2::GetProperty, $
...
IF error NE 0 THEN BEGIN
   ...
   Print, ''
   Print, 'Colorbar2::GetProperty Method: ' + !Error_State.Msg
   RETURN
ENDIF
...
PRO Colorbar2::SetProperty, $

; The SetProperty method of the Colorbar2 object class. All properties are
...
IF error NE 0 THEN BEGIN
   ...
   Print, ''
   Print, 'Colorbar2::SetProperty Method: ' + !Error_State.Msg
   RETURN
ENDIF
...
PRO Colorbar2::LoadCT, index, Draw=draw, Erase=erase

; The LoadCT method of the Colorbar2 object class.
...
IF error NE 0 THEN BEGIN
   ...
   Print, ''
   Print, 'Colorbar2::LoadCT Method: ' + !Error_State.Msg
   RETURN
ENDIF
...
; The Erase method of the Colorbar2 object class.
...
IF error NE 0 THEN BEGIN
   ...
   Print, ''
   Print, 'Colorbar2::Erase Method: ' + !Error_State.Msg
   RETURN
ENDIF
...
PRO Colorbar2::Draw, Erase=erase

; The Draw method of the Colorbar2 object class.
...
IF error NE 0 THEN BEGIN
   ...
   Print, ''
   Print, 'Colorbar2::Draw Method: ' + !Error_State.Msg
   RETURN
ENDIF
...
if self.log then begin
	;ensure that the range is positive and don't plot minor ticks
	self.range = self.range > 1.e-6
	; if range is [1,1] then no pos. data, so set teeny range, so won't crash
	if self.range[0] eq self.range[1] then self.range = [1.,1.001]
	self.minor = 0
endif
...
IF self.vertical THEN BEGIN
   ...
   Plot, [self.range[0], self.range[1]], [self.range[0], self.range[1]], $
      ...
      YRange=self.range, YTickV=*(self.tickv), ylog=self.log
...
ENDIF ELSE BEGIN
   ...
   Plot, [self.range[0], self.range[1]], [self.range[0], self.range[1]], $
      ...
      XTickV=*(self.tickv), xlog=self.log
...
PRO Colorbar2::CleanUp

; The CleanUp method of the Colorbar2 object class.
...
FUNCTION Colorbar2::Init, $

; The INIT method of the Colorbar2 object class.
   ...
   YEraseBox=yerasebox, $   ; A five-element vector of Y points (normalized)
                            ; for erasing the colorbar plot.
   LOG=log                  ; colorbar should be log-scaled
...
IF error NE 0 THEN BEGIN
   ...
   Print, ''
   Print, 'Colorbar2::INIT Method: ' + !Error_State.Msg
   RETURN, 0
ENDIF
...
bar = BytScl(bar, Top=ncolors-1) + bottom
   ...
   ; Fill out this particular instance of the Colorbar2 object.
...
PRO Colorbar2__Define

; Structure definition of the Colorbar2 object class.

struct = { Colorbar2, $     ; The Colorbar2 object class definition.
   ...
   log: 0 $                ; flag to indicate if colorbar should be log-scaled
   }
