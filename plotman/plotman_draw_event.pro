...
;
; Method: Right-click show x,y location of point, and for images, the image value.
; Right-click and drag shows location for start and end and difference.
...
; 16-Mar-2017, Kim. Made it possible to get difference between two locations.  When user right-clicks AND DRAGS,
;  when they release, we now print the difference in IDL log (as well as location of start and end of drag).
;
;-
...
pro plotman_draw_event, event
  ...
  left=0 & middle=0 & right=0 & left_release=0 & right_release=0

  ; Normally we'll ignore release events. Unless it's a right click and release - in that case we'll check
  ; if cursor moved before release, and if so, then print difference between location of click and release.

  case event.press of
    0: if event.release eq 4 then right_release=1 else left_release=1
    1: left=1
    2: middle=1
    4: right=1
    else: begin
  ...
  if left_release eq 1 then begin
    state.plotman_obj -> unselect
  ...
  if right or right_release then begin
    ...
    if right_release and same_data(lastclick, {x: xs, y: ys, z: image_val}) then return
    ...
    if right_release then print, 'Difference x,y,z = ', plotman_click_diff()
