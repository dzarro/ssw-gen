...
; 12-Apr-2017, Kim. Added full keyword to panel_summary. And in hide_all_panels, check that w_draw is alive
;   before reducing it's size (user may have deleted window id outside of plotman)
;
;-----
  ...
  if nump gt 0 then begin
    for ip = 0,nump-1 do begin
      ...
      if xalive((*panel).w_drawbase) then begin
        ...
        if xalive((*panel).w_draw) then begin
...
pro plotman::panel_summary, full=full
  ...
  if nump gt 0 then begin
    for ip = 0,nump-1 do begin
      ...
      if full && p.saved_data.save_mode eq 'obj_extract' then begin
