...
pro plot_map_index,map,contour=cont,overlay=overlay,smooth_width=smooth_width,border=border,$
 ...
 red=red,green=green,blue=blue,about_center=about_center,$
...
if keyword_set(overlay) && (n_params() eq 0) then begin
 ...
 if about_center then rcenter=last_center else rcenter=last_rcenter
 plot_helio,last_time,roll=last_roll,grid_spacing=grid_spacing,$
  /over,rcenter=rcenter,$
...
log_warn_msg='All data are negative. Cannot plot on a log scale. Using linear scale.'
if dlog then begin
 ok=where(pic gt 0.,pcount)
 if pcount eq 0 then begin
  mprint,log_warn_msg,/info
  dlog=0b
 endif
 if dlog then begin
...
units=get_map_prop(map,/units,def='arcsec')
...
b0=angles.b0 & b0=b0[0]
l0=angles.l0 & l0=l0[0]
rsun=angles.rsun & rsun=rsun[0]
...
if cont then begin
 if over then begin
  ...
  dum=drot_map(map,time=last_time,trans=trans,$
   ...
   rcenter=rcenter,err=err_msg,xp=xp,yp=yp,/no_data,ilimb=ilimb,$
   no_drotate=no_drotate,no_project=no_project,$
   no_roll_correct=no_roll_correct,about_center=about_center,$
    _extra=extra)
...
if ~over then begin
 ...
 if exist(icenter) then last_center=icenter
