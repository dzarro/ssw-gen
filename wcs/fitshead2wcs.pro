...
;
;  Filter out any tags with a string value of 'NaN' or 'nan'
;
for i=0,n_elements(tags)-1 do begin
    val = index.(i)
    if (datatype(val) eq 'STR') and (n_elements(val) eq 1) then $
      if strlowcase(val) eq 'nan' then tags[i] = ''
endfor
;
...
for i=1,n_axis do begin
    val = wcs_find_keyword(index, tags, column, system, count, $
                           'CROTA'+ntrim(i), ntrim(i)+'CROT', lunfxb=lunfxb, $
                           rowfxb=rowfxb)
